...
vector<pair<pair<int, int>, int>> calculatePrimsMST(int n, int m, vector<pair<pair<int, int>, int>> &edges)
{
    ...
    for(int i=1;i<n;i++){
        ...
        for(int j=1;j<=n;j++){           //finding minimum node
            if(!mst[j] && key[j]<mini){
                u=j;
                mini=key[j];
            }
        ...
        for(auto neigh:adj[u]){
            ...
            if(!mst[val] && weight < key[val]){       //value included nhi h mst me aur curr weight kam h 
                parent[val]=u;
                key[val]=weight;
            }
        }
    ...
    for(int i=2;i<=n;i++){      //1 ka parent -1 h isliye 2 se
...
#include <bits/stdc++.h>
void dfs(unordered_map<int,vector<int>>&adj,vector<int>&vis,int ele){
	vis[ele]=true;
...
void topoSort(unordered_map<int,vector<int>>&adj,vector<int>&vis,int ele,stack<int>&st){
	vis[ele]=true;
...
// check if there is a cycle in a matrix

class Solution {
public:
    vector<vector<int>>dirs = {{1,0}, {-1, 0}, {0, 1}, {0, -1}};
    bool dfs(vector<vector<char>>& grid,vector<vector<bool>>& vis, int i, int j ,int pi, int pj){
        vis[i][j] = true;
        int n = grid.size();
        int m = grid[0].size();
        for(auto &d : dirs){
